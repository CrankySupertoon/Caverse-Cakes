...
public class BlockCustomCake extends BlockCakeBase implements ITileEntityProvider {
    ...
    public BlockCustomCake() {
        super();
        setRegistryName(CaverseCakes.MODID + ":custom_cake");
        setTranslationKey(CaverseCakes.MODID + ".custom_cake");
    }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        for (String s : ModConfig.customEdible.customCake.fuel) {
            try {
                ...
                if (parts.length < 2) {
                    CaverseCakes.logger.log(Level.ERROR, s + " is not a valid input line! Format needs to be: <dimID>, <cakeFuel>");
                    continue;
                }
